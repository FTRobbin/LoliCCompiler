...
public class CodeSelect {
    ...
    void saveGlobalUnbonded() {
        for (VarName var : vars.keySet()) {
            if (var.uid == 0 && !vars.get(var).inmem && !var.isTemp && !envr.bond.containsKey(var)) {
                ...
                code.addText(new SPIMInst(var.size == 1 ? SPIMOp.sb : SPIMOp.sw, reg, getAddr(var)));
    ...
    void saveCall(SPIMLabel func) {
        ...
        for (SPIMRegister reg : regs.keySet()) {
            ...
            for (VarName var : regSt.vars) {
                ...
                if (func == null || func.equals(mySelf) || reg instanceof SPIMPhysicalRegister) {
                    removed.add(var);
                    if ((var.uid == 0 || curlive.contains(var)) && !vars.get(var).inmem) {
    ...
    void saveNonArguBondRegs(HashSet<VarName> live) {
        for (VarName var : live) {
            ...
            code.addText(new SPIMInst(var.size == 1 ? SPIMOp.sb : SPIMOp.sw, vars.get(var).regs.iterator().next(), getAddr(var)));
            vars.get(var).inmem = true;
        }
    }
    ...
    SPIMAddress getAddr(VarName var) {
        if (var instanceof DeRefVar) {
            VarName val = (VarName)((DeRefVar) var).val;
            if (val.isArray || val.isStruct || val.isFunc) {
                return getAddr(val);
            } else {
    ...
    SPIMAddress getInfAddr() {
        curDelta = alignTo(curDelta, 4) + 4;
    ...
    void saveReg(SPIMRegister reg) {
        if (regs.containsKey(reg)) {
            ...
            if (alias > 2) {
                SPIMRegister reg1 = new SPIMInfRegister(getInfAddr());
    ...
    SPIMRegister writeResultReg(VarName var) {
        ...
        if (envr.bond.containsKey(var)) {
            ...
            reg = new SPIMInfRegister(getInfAddr());
        }
        if (!regs.containsKey(reg)) {
            regs.put(reg, new RegisterStatus());
        }
    ...
    SPIMRegister loadToRegT0(Value val) {
        //In registers
        if (vars.containsKey(val) && vars.get(val).regs.size() > 0) {
            return vars.get(val).regs.iterator().next();
        }
    ...
    void genInst(AssignInst inst) {
        if (inst.op.equals(ExprOp.asgr)) {
            ...
            SPIMAddress src = ((VarName)inst.src1).isTemp ? new SPIMAddress(loadToReg((VarName)inst.src1)) : getAddr((VarName)inst.src1),
                        dst = getAddr(inst.dest);
            SPIMRegister reg = new SPIMInfRegister(getInfAddr());
            //System.out.println(src.print() + " / " + dst.print() + " / " + reg.print());
            ...
            if (inst.dest instanceof DeRefVar) {
                ...
                if (inst.op.equals(ExprOp.asg)) {
                    code.addText(new SPIMInst(inst.dest.size == 1 ? SPIMOp.sb : SPIMOp.sw, loadToReg(inst.src1), adr));
                    ...
                    if (cnt == 1) {
                        ...
                        list.push(new SPIMInfRegister(getInfAddr()));
                        code.addText(new SPIMInst(SPIMOp.valueOf(inst.op.name()), list.get(0), list.get(1)));
                        code.addText(new SPIMInst(inst.dest.size == 1 ? SPIMOp.sb : SPIMOp.sw, list.get(0), adr));
                    } else if (cnt == 2) {
                        Value val = inst.src2;
                        if (val instanceof IntConst) {
                            LinkedList<SPIMRegister> list = new LinkedList<>();
    ...
    void genInst(CallInst inst) {
        ...
        if (inst.func.name.equals("___getchar")) {
            ...
            if (inst.dest != null) {
                if (inst.dest instanceof DeRefVar) {
                    code.addText(new SPIMInst(((DeRefVar) inst.dest).size == 1 ? SPIMOp.sb : SPIMOp.sw, SPIMRegID.$v0.getReg(), getAddr(inst.dest)));
                } else {
        ...
        for (Value val : pStack) {
            ...
            if (val instanceof VarName && ((VarName) val).isStruct) {
                ...
                SPIMRegister reg = SPIMRegID.$t0.getReg();
                if (val.align == 1) {
                ...
                if (paraCnt < 4 && !(inst.func.name.equals("___printf"))) {
                    ...
                    code.addText(new SPIMInst(val.size == 1 ? SPIMOp.sb : SPIMOp.sw, reg, new SPIMAddress(SPIMImmediate.getImmi(-paraAdr), SPIMRegID.$sp.getReg())));
                }
            }
            ++paraCnt;
        }
    ...
    void genInst(MemInst inst) {
        ...
        if (envr.bond.containsKey(inst.var)) {
            ...
            if (reg instanceof SPIMInfRegister && ((SPIMInfRegister) reg).addr == null) {
                ((SPIMInfRegister) reg).addr = vars.get(inst.var).mem;
            }
        }
    }
