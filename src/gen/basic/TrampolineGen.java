 ...
 */
public class TrampolineGen implements CodeGen {
    ...
    void genInst(MIRInst inst) {
        ...
        */
        ...
        } else if (inst instanceof TrampInst) {
            genInst((TrampInst)inst);
        }
    }
    ...
    void genInst(TrampInst inst) {
        alignTo(4);
        curDelta += inst.tramped.size;
        delta.put(inst.tramped, -curDelta);
        code.addText("\t\tla\t$t0\t" + -(curDelta + 20) + "($fp)");
        code.addText("\t\tsw\t$t0\t" + getAddr(inst.tramped));
        curDelta += 20;
        code.addText("\t\t#Trampoline");
        code.addText("\t\t#Suppose current framepointer $fp is 0xXXXXYYYY");
        code.addText("\t\tli\t$t0\t0x3c01\t#lui\t$1\t0xXXXX");
        code.addText("\t\tsll\t$t0\t4");
        code.addText("\t\tsrr\t$t1\t$fp\t4");
        code.addText("\t\tadd\t$t0\t$t1");
        code.addText("\t\tsw\t$t0\t" + -curDelta + "$(fp)");
        curDelta -= 4;
        code.addText("\t\tli\t$t0\t0x343d\t#ori\t$sp\t$1\t0xYYYY");
        code.addText("\t\tsll\t$t0\t4");
        code.addText("\t\tand\t$t1\t$fp\t0xffff");
        code.addText("\t\tadd\t$t0\t$t1");
        code.addText("\t\tsw\t$t0\t" + -curDelta + "$(fp)");
        curDelta -= 4;
        code.addText("\t\tla\t$t2\t" + inst.func.name + "\t#j " + inst.func.name);
        code.addText("\t\tli\t$t0\t0x3c01");
        code.addText("\t\tsll\t$t0\t4");
        code.addText("\t\tsrr\t$t1\t$t2\t4");
        code.addText("\t\tadd\t$t0\t$t1");
        code.addText("\t\tsw\t$t0\t" + -curDelta + "$(fp)");
        curDelta -= 4;
        code.addText("\t\tli\t$t0\t0x3429");
        code.addText("\t\tsll\t$t0\t4");
        code.addText("\t\tand\t$t1\t$t2\t0xffff");
        code.addText("\t\tadd\t$t0\t$t1");
        code.addText("\t\tsw\t$t0\t" + -curDelta + "$(fp)");
        curDelta -= 4;
        code.addText("\t\tli\t$t0\t0x01200008");
        code.addText("\t\tsw\t$t0\t" + -curDelta + "$(fp)");
        curDelta -= 4;
        curDelta += 20;
    }
